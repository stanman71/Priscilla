            ...
            if group != None:
                ...
                if group.light_ieeeAddr_1 != "None":
                    ...
                    if scene != None:
                        ...
                        WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Scene - " + task[3] + " | missing")

                else:
                    WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Group - " + task[2] + " | empty")

            else:
                WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Group - " + task[2] + " | missing")
    ...
    try:
        if "lighting" in task_object.task and "light" in task_object.task and "start_scene" not in task_object.task and "turn_off" not in task_object.task:
            ...
            if device != None:
                ...
                WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Light - " + task[2] + " | missing")
        ...
        if "lighting" in task_object.task and "turn_off" in task_object.task:
                ...
                if group.light_ieeeAddr_1 != "None":
                    ...
                    for input_group_name in list_groups:
                        ...
                        if group_found == False:
                            WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Group - " + input_group_name + " | missing")

                else:
                    WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Group - " + input_group_name + " | empty")
            ...
            if task[2].lower() == "light":
                ...
                if device != None:
                    ...
                    WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Light - " + task[3].strip() + " | missing")
            ...
            if task[2].lower() == "all":
                ...
                for group in GET_ALL_LIGHTING_GROUPS():
                    ...
                    if group.light_ieeeAddr_1 != "None":
                        ...
                        WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Group - " + group.name + " | empty")
        ...
        if "device" in task_object.task and "update" not in task_object.task:
            ...
            for device_name in task[1].split(","):
                ...
                if device != None:
                    ...
                    WRITE_LOGFILE_SYSTEM("ERROR", "Scheduler | Task - " + task_object.name + " | Device - " + device_name.strip() + " | missing")
